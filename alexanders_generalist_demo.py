...
def evaluate(env, x):
    ...
    both_f = [simulation(env, individual) for individual in tqdm(x, desc=f"Evaluating Population: ", leave=False)]
    f = {
...
def evolution_step(env, pop, pfit, mutation_rate, mutation_type, fitness_method, 
                   pick_parent_method, survivor_method, crossover_method, dom_upper, dom_lower,
                  _gen):
    ...
    print(f'\nStats: Inputs:')
    print(f'>>>(0) BEST PFIT IN POP_NEW: {np.max(pfit["balanced"]):.2f} (Balanced)')
    print(f'>>>(0) BEST PFIT IN POP_NEW: {np.max(pfit["default"]):.2f} (Default)')    
    # Normalize fitnesses
    ...
    if mutation_type == 'normal':
        ...
        # Stochastic Noise
        starting_std = 0.5  # Replace with your desired starting value
        ...
        # Evaluate new population
        pfit_new = evaluate(env, pop_new)
    elif mutation_type == 'KOH':
        # generate std array to mutate with
        # stds = np.array([0.0, 0.001, 0.002, 0.004, 0.008, 0.016, 0.032, 0.064, 0.0128, 0.0256, 0.0512, 0.1024, 0.2048])
        stds = np.array([0.0, 0.001, 0.01, 0.1])
        pfit_scores = np.zeros_like(stds)
        pop_new_storage = []
        pfit_new_storage = []
        from tqdm import tqdm
        for it, std in tqdm(enumerate(stds), desc='KOH Iterations', leave=False):
            # np.random.seed(_gen)
            # Mutate
            pop_new = mutate(pop_new, mutation_rate, std=std)
            # Clip to domain
            pop_new = np.clip(pop_new, -1, 1)
            pop_new_storage.append(pop_new)
            # Evaluate new population
            pfit_new = evaluate(env, pop_new)
            pfit_new_storage.append(pfit_new)
            pfit_scores[it] = np.max(pfit_new["default"])
        
        # DF with std and pfit
        std_df = pd.DataFrame({'std': stds, 'pfit': pfit_scores})
        print(std_df.T.to_string(float_format='%.4f'))
        print(f'>>Best std: {stds[np.argmax(pfit_scores)]:.4f}, pfit: {np.max(pfit_scores):.4f}')
        ZeroSTD_FAG = True if stds[np.argmax(pfit_scores)] == 0 else False
        # select best pop_new
        pop_new = pop_new_storage[np.argmax(pfit_scores)]
        pfit_new = pfit_new_storage[np.argmax(pfit_scores)]
        del pop_new_storage, pfit_scores, stds, std_df, pfit_new_storage

        print(f'\nStats: Childern - Mutation:')
        print(f'>>>(1) BEST PFIT IN POP_NEW: {np.max(pfit_new["balanced"]):.2f} (Balanced)')
        print(f'>>>(1) BEST PFIT IN POP_NEW: {np.max(pfit_new["default"]):.2f} (Default)')

    if (mutation_type == 'KOH' and not ZeroSTD_FAG) or  mutation_type == 'normal' or mutation_type == 'stochastic_decaying' :
        ...
        # store best agent based on pfit_combined "balanced" to append at the end
        best_pop_idx  = np.argmax(pfit_combined['balanced'])
        pop_best  = pop_combined[best_pop_idx]
        pfit_best = (pfit_combined['default'][best_pop_idx], pfit_combined['balanced'][best_pop_idx])
        ...
        # check if pop best is in pop_new
        if not np.any(np.all(pop_new == pop_best, axis=1)):
            # append best agent to pop_new
            pop_new = np.vstack((pop_new, pop_best))
            pfit_new = {
                "default": np.append(pfit_new["default"], pfit_best[0]),
                "balanced": np.append(pfit_new["balanced"], pfit_best[1]),
            }
        print(f'\nStats: Pop after combining with parents:')
        print(f'>>>(2) BEST PFIT IN POP_NEW: {np.max(pfit_new["balanced"]):.2f} (Balanced)')
        print(f'>>>(2) BEST PFIT IN POP_NEW: {np.max(pfit_new["default"]):.2f} (Default)')
        # Return new population and fitnesses
        return pop_new, pfit_new
    elif mutation_type == 'KOH' and ZeroSTD_FAG :
        print('>>WARNING: Failed to mutate. Reverting back to previous gen. \n')
        print(f'\nAfter combining with parents:')
        print(f'>>>(2) BEST PFIT IN POP_NEW: {np.max(pfit_new["balanced"]):.2f} (Balanced)')
        print(f'>>>(2) BEST PFIT IN POP_NEW: {np.max(pfit_new["default"]):.2f} (Default)')
        return pop, pfit
...
def main(
        ...
        mutation_type = 'stochastic_decaying', # 'normal', 'stochastic_decaying', 'KOH'
        start_new = True,
        # Alexander's Variables
        initial_pop_size = 1_000,
):
    # Remove old options
    if multi_ini:
    ...
    if n_hidden_neurons > 0:    n_vars = (env.get_num_sensors()+1)*n_hidden_neurons + (n_hidden_neurons+1)*5
    else:                       n_vars = (env.get_num_sensors()+1)*5

    # Load population
    # np.random.seed(0)
    pop, pfit = load_population(domain_lower, domain_upper, initial_pop_size, n_vars, env, evaluate, fitness_method, use_folder, continue_evo=start_gen>0)
    print(f'Best pfit: {np.max(pfit["balanced"]):.2f} (Balanced)')
    print(f'Worst pfit: {np.min(pfit["balanced"]):.2f} (Balanced)')
    # Keep the best 100 of that population    

    argsortf100 = np.argsort(pfit['balanced'])[-pop_size:]
    pop = pop[argsortf100] 
    pfit['default'] = pfit['default'][argsortf100] 
    pfit['balanced'] = pfit['balanced'][argsortf100] 
    print(f'Worst pfit after top100 filtering: {np.min(pfit["balanced"]):.2f} (Balanced)')
    del argsortf100
    ...
    for gen in range(start_gen, gens):
        ...
        pop, pfit = evolution_step(env, pop, pfit, mutation_rate, mutation_type=mutation_type, fitness_method=fitness_method, 
                                   ...
                                   crossover_method=crossover_method, dom_upper=domain_upper, dom_lower=domain_lower,
                                   _gen=gen
                                )
        # Trim population back to pop_size based on pfit
        if len(pop) > pop_size:
            # Select best
            idx = np.argsort(pfit["balanced"])[::-1][:pop_size]
            pop = pop[idx]
            pfit["default"] = pfit["default"][idx]
            pfit["balanced"] = pfit["balanced"][idx]
...
if __name__ == '__main__':
    config = {
        ...
        "enemies":              [7, 8],                # [1, 2, 3, 4, 5, 6, 7, 8], [7,8]
        ...
        "mutation_type":        "KOH",      # "stochastic_decaying", "normal", "KOH"
        "gens":                 300,
        "pop_size":             100,
